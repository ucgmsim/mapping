...
def get_args():
    ...
    arg(
        "--remove_not_in_basin",
        help="Remove stations not in basin",
        action="store_true",
    )
...
if __name__ == "__main__":
    args = get_args()
    try:
        basin_outlines = basin_outlines_dict[arg.version]
    except KeyError:
    ...
    if args.stat_file is None:
        ...
        ll_points = np.column_stack(
            (land_lon, land_lat)
        )  # zip them into an array of (lon, lat) pairs
        stat_names = None
    else:
    ...
    df = pd.DataFrame(
        {
            ...
            "lon": ll_points[:, 0],
            "lat": ll_points[:, 1],
            "NZGD_lon": nztm_points[:, 0],
            "NZGD_lat": nztm_points[:, 1],
        }
    )
